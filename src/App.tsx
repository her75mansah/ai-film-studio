...
const appId =
  typeof __app_id !== 'undefined' ? __app_id : 'ai-film-studio-vercel';
...
const getStyleModifiers = (style, subStyle = 'Detail') => {
  ...
  const baseLock =
    ', strictly consistent art style, unified visual identity, masterpiece';
  if (s.includes('pixar') || s.includes('3d'))
    return (
      baseLock +
      ', Pixar 3D animation style, Disney 3D, cute stylized 3D render, smooth volumetric lighting, vibrant colors'
    );

  if (
    s.includes('2d classic') ||
    s.includes('cartoon') ||
    s.includes('comic')
  ) {
    ...
    if (sub.includes('minimal')) {
      return (
        baseLock +
        ', minimalist comic book style, simple clean lines, minimal details, flat simple shading, completely flat 2d vector art, no 3d render'
      );
    } else if (sub.includes('realistis')) {
      return (
        baseLock +
        ', realistic comic book style, graphic novel realism, detailed facial features, realistic proportions with comic shading, cel shaded realism, completely flat 2d, no 3d render'
      );
    } else if (sub.includes('exaggerated') || sub.includes('lebih')) {
      return (
        baseLock +
        ', exaggerated comic book style, dynamic dramatic proportions, extreme perspective, caricature style, highly expressive, stylized flat 2d, crisp bold black outlines, no 3d render'
      );
    } else {
      return (
        baseLock +
        ', highly detailed comic book illustration, intricate ink lines, complex graphic novel shading, vivid colors, crisp bold black outlines, completely flat 2d, no 3d render'
      );
    }
  }

  if (s.includes('ghibli') || s.includes('anime') || s.includes('manga'))
    return (
      baseLock +
      ', Studio Ghibli style, anime art style, 2D Japanese animation, beautiful painted background, cel shaded, ethereal, completely flat 2d'
    );
  if (s.includes('claymation') || s.includes('plasticin'))
    return (
      baseLock +
      ', claymation style, Aardman animation, plasticine texture, stop motion animation, macro photography, tactile texture, miniature studio lighting'
    );
  if (s.includes('watercolor') || s.includes('painted'))
    return (
      baseLock +
      ', watercolor painting, ink and wash, artistic, ethereal, visible brush strokes'
    );
  if (s.includes('vintage'))
    return (
      baseLock +
      ', vintage photography, Kodak Portra 400, grainy, cinematic 35mm lens, nostalgic, retro aesthetic'
    );
  if (s.includes('cyberpunk'))
    return (
      baseLock +
      ', cyberpunk art style, neon glowing lights, futuristic, highly detailed, octane render, vivid sci-fi colors'
    );
...
const rewriteDialogueText = async (baseDialogue, genre, idea) => {
  ...
  try {
    ...
    const data = await response.json();
    return (
      data.candidates?.[0]?.content?.parts?.[0]?.text.trim() || baseDialogue
    );
  } catch (e) {
    console.error(e);
    return baseDialogue;
...
const generateCastingSuggestions = async (char) => {
  ...
  try {
    ...
    return (
      ...
      'Gagal mendapatkan saran casting.'
    );
  } catch (e) {
    console.error(e);
    ...
    try {
      if (isImageToImage) {
        ...
        try {
          ...
          console.warn(
            'Imagen4 failed, falling back to Gemini Flash Image:',
            e
          );
          return await callGeminiFlashImage();
        }
...
const generateFilmPipeline = async (formData, onProgress) => {
  const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`;
  const fullStyle =
    formData.style === 'Cartoon / Comic Book' && formData.subStyle
      ? `${formData.style} - ${formData.subStyle}`
      : formData.style;
...
const InputField = ({
  ...
  <div className="mb-4">
    <label
      htmlFor={id}
      className="block text-sm font-medium text-gray-400 mb-1 tracking-wide uppercase text-xs"
    >
      {label}
    </label>
    <input
...
const AspectRatioSelector = ({ id, value, onChange }) => {
  const options = [
    ...
    },
    {
      label: '2.35:1',
      subLabel: 'Cinematic',
      value: '2.35:1 (Cinematic Widescreen)',
      boxClass: 'w-8 h-3.5',
    },
    {
      label: '9:16',
      ...
      <div className="grid grid-cols-2 gap-3">
        {options.map((opt) => {
          ...
          return (
            ...
            >
              <div
                className={`border-2 mb-2 rounded-sm transition-colors ${
...
const renderMarkdown = (text) => {
  ...
  if (processedText.includes('|')) {
    ...
    const processTableBlock = (rows) => {
      ...
      let tableHtml =
        '<div className="overflow-x-auto my-6 border border-gray-800 rounded-lg shadow-xl break-inside-avoid"><table className="w-full text-sm text-left"><thead className="text-xs uppercase bg-gray-900 text-blue-400 border-b border-gray-800">';
      let isHead = true;
      ...
      rows.forEach((row) => {
        if (row.includes('---')) {
          isHead = false;
          tableHtml +=
            '</thead><tbody className="divide-y divide-gray-800 bg-gray-900/30">';
          return;
        }
        ...
        if (isHead) {
          ...
          cells.forEach(
            (cell) =>
              (tableHtml += `<th className="px-4 py-4 font-bold tracking-wider bg-gray-900 border-r border-gray-800 last:border-r-0">${cell.trim()}</th>`)
          );
          tableHtml += '</tr>';
        } else {
          tableHtml +=
          ...
          cells.forEach(
            (cell) =>
              (tableHtml += `<td className="px-4 py-3 align-top text-gray-300 min-w-[100px] border-r border-gray-800 last:border-r-0 leading-relaxed">${cell.trim()}</td>`)
          );
          tableHtml += '</tr>';
        }
      });
  ...
  let html = processedText
    .replace(
      /^### (.*$)/gim,
      '<h3 className="text-xl font-bold mt-6 mb-2 text-blue-300">$1</h3>'
    )
    .replace(
      /^## (.*$)/gim,
      '<h2 className="text-2xl font-bold mt-8 mb-4 text-blue-400 border-b border-gray-800 pb-2">$1</h2>'
    )
    .replace(
      /^# (.*$)/gim,
      '<h1 className="text-3xl font-bold mt-8 mb-4 text-white">$1</h1>'
    )
    .replace(
      /\*\*(.*?)\*\*/g,
      '<strong className="text-white font-semibold">$1</strong>'
    )
    .replace(/\*(.*?)\*/g, '<em className="text-gray-300 italic">$1</em>')
    .replace(
      /^\s*-\s(.*)$/gim,
      '<li className="ml-4 list-disc marker:text-blue-500 mb-1">$1</li>'
    );
  ...
  const formattedBlocks = blocks.map((block) => {
    if (
      ...
      return `<p className="mb-4 leading-relaxed text-gray-300">${block
        .trim()
        .replace(/\n/g, '<br />')}</p>`;
    }
  });
...
const CharacterCard = ({ char }) => {
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 flex flex-col break-inside-avoid shadow-lg hover:border-blue-500/50 transition-colors">
      ...
      {char.role && (
        <span className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-4 inline-block bg-gray-800 px-2 py-1 rounded">
          {char.role}
        </span>
      )}
      ...
      <div className="mb-5 bg-gradient-to-r from-gray-950 to-indigo-950/30 p-3 rounded-lg border border-indigo-500/30 no-print">
        {!casting && !isCasting ? (
          ...
          <div className="text-xs text-indigo-200">
            <strong className="flex items-center gap-1 text-indigo-400 mb-1">
              <SparklesIcon className="w-3 h-3" /> Saran Casting:
            </strong>
      ...
      <div className="mt-auto bg-black/60 p-3 rounded-lg border border-gray-800 relative group">
        <div className="flex justify-between items-center mb-2">
          <span className="text-[10px] text-green-400 font-bold uppercase tracking-wider flex items-center gap-1">
            <SparklesIcon className="w-3 h-3" /> Saran Prompt AI
          </span>
          <button
            onClick={() => handleCopyPrompt(char.prompt)}
            className="text-gray-500 hover:text-white bg-gray-800 hover:bg-gray-700 p-1.5 rounded transition-colors no-print"
            title="Copy Prompt"
          >
            <CopyIcon className="w-3 h-3" />
          </button>
        </div>
        <p className="text-xs text-gray-400 italic line-clamp-3 group-hover:line-clamp-none transition-all duration-300">
          {char.prompt}
        </p>
      </div>
    </div>
...
const ImageLightbox = ({
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-8 bg-black/95 backdrop-blur-md transition-all no-print"
      onClick={onClose}
    >
      <div
        className="relative flex flex-col items-center justify-center max-w-full max-h-full w-full h-full"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={onClose}
          disabled={isGenerating}
          className={`absolute top-4 right-4 sm:top-8 sm:right-8 p-3 text-gray-400 hover:text-white bg-gray-900/50 hover:bg-red-500/80 rounded-full transition-all z-10 ${
        ...
        <div className="relative max-w-full max-h-[75vh] flex items-center justify-center group">
          <img
            src={imageUrl}
            alt="Enlarged Visual"
            className={`max-w-full max-h-[75vh] object-contain rounded-xl shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-gray-800 transition-all duration-300 ${
              isGenerating
        ...
        <div
          className={`mt-8 flex items-center gap-6 bg-gray-900/90 backdrop-blur-md border border-gray-700 px-8 py-4 rounded-full shadow-2xl transition-opacity duration-300 ${
...
const StoryboardHeaderWidget = ({
  ...
  const locSection = sections?.find(
    (s) => s.id === '6' || s.title.includes('LOCATION')
  );
  let locSuggestions = [];
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const globalContext = `[Context: ${
        formData.genre
      } genre film, Theme: ${formData.idea.substring(
        0,
        100
      )}, Style: ${fullStyle}] `;
      ...
      const finalPrompt =
        ...
        styleBooster +
        ', 1:1 aspect ratio, square orientation';
  ...
  const handleGenerateLoc = async () => {
    ...
    try {
      ...
      const finalPrompt =
        ...
        globalContext +
        locPrompt.trim() +
        ', environment concept art, empty environment, no people' +
        styleBooster +
        ', 16:9 aspect ratio';
    ...
    <div className="mb-8 p-6 bg-gradient-to-br from-gray-900 to-gray-950 border border-blue-500/30 rounded-xl shadow-lg relative overflow-hidden break-inside-avoid">
      ...
      <div className="mb-5">
        ...
        <p className="text-sm text-gray-400 mt-1 leading-relaxed max-w-3xl">
          Tambahkan hingga 6 referensi wajah karakter utama (via teks AI atau
          upload foto). Anda bisa memilih karakter mana yang ingin dimunculkan
          di setiap adegan Storyboard di bawah nanti!
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3 mb-5">
        {characterBank.map((img, index) => (
          <div
            key={index}
            className="relative group aspect-square rounded-lg border-2 border-green-500 shadow-[0_0_15px_rgba(34,197,94,0.2)] bg-black overflow-hidden cursor-pointer"
          ...
          >
            ...
            <div className="absolute bottom-1 left-1 bg-black/70 text-xs font-bold text-green-400 px-1.5 rounded pointer-events-none">
              #{index + 1}
            </div>
          </div>
        ))}
        ...
        {Array.from({ length: 6 - characterBank.length }).map((_, i) => (
          <div
            key={`empty-${i}`}
            className="aspect-square border-2 border-dashed border-gray-700/50 rounded-lg flex items-center justify-center bg-gray-900/30"
          >
            <span className="text-xs font-medium text-gray-600 uppercase tracking-widest">
              Slot {characterBank.length + i + 1}
            </span>
          </div>
        ))}
      </div>
          ...
          <div className="flex items-center gap-3">
            {charRefImage && (
              <div className="relative w-12 h-12 rounded overflow-hidden border border-blue-500 shrink-0 shadow-lg">
                ...
                />
                <button
                  onClick={() => setCharRefImage(null)}
                  className="absolute top-0 right-0 bg-red-500/80 hover:bg-red-500 text-white p-0.5 rounded-bl transition-colors"
                  title="Hapus Ref"
                >
            ...
            <div className="flex-1 w-full relative flex items-center bg-black border border-gray-700 rounded-md focus-within:border-blue-500 transition-colors">
              ...
              />
              <label
                className="absolute right-2 cursor-pointer text-gray-400 hover:text-blue-400 p-1"
                title="Tambah Gambar Referensi untuk Prompt"
              >
      ...
      {error && (
        <p className="text-red-400 text-sm mt-3 font-medium bg-red-900/20 p-2 rounded border border-red-800">
          {error}
        </p>
      )}
      ...
      <div className="mb-5 mt-10 pt-8 border-t border-gray-800/80">
        ...
        <p className="text-sm text-gray-400 mt-1 leading-relaxed max-w-3xl">
          Tambahkan hingga 4 referensi lokasi. Anda bisa mengetik prompt dan
          menyertakan foto (*image-to-image*) lalu *Generate*, atau langsung
          *Upload* latar belakang yang sudah ada.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {locationBank.map((img, index) => (
          <div
            key={index}
            className="relative group aspect-video rounded-lg border-2 border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.2)] bg-black overflow-hidden cursor-pointer"
            onClick={() => setPreviewImage(img)}
          >
            <img
              src={img}
            ...
            <div className="absolute bottom-1 left-1 bg-black/70 text-xs font-bold text-emerald-400 px-1.5 rounded pointer-events-none">
              L{index + 1}
            </div>
          </div>
        ))}
        ...
        {Array.from({ length: 4 - locationBank.length }).map((_, i) => (
          <div
            key={`empty-loc-${i}`}
            className="aspect-video border-2 border-dashed border-gray-700/50 rounded-lg flex items-center justify-center bg-gray-900/30"
          >
            <span className="text-xs font-medium text-gray-600 uppercase tracking-widest">
              Loc {locationBank.length + i + 1}
            </span>
          </div>
        ))}
      </div>
        ...
        <div className="flex flex-col gap-3 bg-gray-900 p-3 rounded-lg border border-gray-800 shadow-inner no-print">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 w-full relative flex items-center bg-black border border-gray-700 rounded-md focus-within:border-emerald-500 transition-colors">
              ...
              />
              <label
                className="absolute right-2 cursor-pointer text-gray-400 hover:text-emerald-400 p-1"
                title="Tambah Gambar Referensi untuk Prompt"
              >
                <PlusIcon className="w-5 h-5" />
                <input
            ...
            <span className="text-gray-600 text-xs font-black uppercase tracking-widest hidden xl:block">
              Atau
            </span>
            <label className="hidden xl:flex cursor-pointer px-4 py-2.5 bg-blue-600/20 hover:bg-blue-600/40 border border-blue-500/50 rounded-md text-sm font-bold text-blue-300 transition-colors items-center justify-center gap-2 shrink-0">
              ...
              <input
                ...
                accept="image/*"
                className="hidden"
                onChange={handleDirectLocUpload}
              />
            </label>
          </div>
      ...
      {locSuggestions.length > 0 && (
        <div className="mt-8 pt-6 border-t border-gray-800/80 no-print">
          <h5 className="text-sm text-emerald-400 font-bold uppercase tracking-widest mb-4 flex items-center gap-2">
            <SparklesIcon className="w-4 h-4" /> Referensi Prompt Lokasi (Dari
            Naskah)
          </h5>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            {locSuggestions.map((sug, idx) => {
              ...
              return (
                <div
                  key={`sug-${idx}`}
                  className="bg-black/50 border border-gray-800 rounded-lg p-4 flex flex-col justify-between hover:border-emerald-500/50 transition-all duration-300 group shadow-lg"
                >
                  <div className="mb-4">
                    <strong className="text-emerald-400 text-[13px] block mb-2 uppercase tracking-wider">
                      {title}
                    </strong>
                    <p
                      className="text-[11px] text-gray-400 line-clamp-4 leading-relaxed"
                      title={desc}
                    >
                      {desc}
                    </p>
      ...
      <ImageLightbox
        ...
        onDownload={() =>
          handleDownloadUpscaled(previewImage, `reference-HD.png`)
        }
        onDelete={() => {
...
const SceneStoryboardPanel = ({
  ...
  const visualMatch = content.match(
    /Visual:\s*([\s\S]*?)(?=Motion:|Audio:|Info:|$)/i
  );
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      const isOutsideChar =
        charSelectorRef.current &&
        !charSelectorRef.current.contains(event.target);
      const isOutsideLoc =
        locSelectorRef.current &&
        !locSelectorRef.current.contains(event.target);
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      ...
      if (aspectRatio) {
        if (aspectRatio.includes('16:9'))
          ratioInstruction =
            ', 16:9 aspect ratio, widescreen landscape orientation';
        else if (aspectRatio.includes('2.35:1'))
          ratioInstruction =
            ', 2.35:1 aspect ratio, ultra-widescreen cinematic orientation';
        else if (aspectRatio.includes('9:16'))
          ratioInstruction =
  ...
  const handleGenerateAudio = async () => {
    ...
    setAudioError(null);
    try {
      const url = await generateAudioContent(
        audioPrompt.trim(),
        formData.voiceStyle
      );
      setAudioUrl(url);
  ...
  const handleRewriteAudio = async () => {
    ...
    setAudioError(null);
    try {
      const newDialogue = await rewriteDialogueText(
        audioPrompt,
        formData.genre,
        formData.idea
      );
  ...
  return (
    <div className="my-8 bg-gray-900 border border-gray-800 rounded-xl shadow-xl overflow-hidden flex flex-col md:flex-row w-full group relative break-inside-avoid print-flex-col print-card">
        ...
        <div className="grid grid-cols-1 gap-4 flex-1">
          <div className="flex flex-col gap-2">
            {characterBank && characterBank.length > 0 && (
              <div
                ref={charSelectorRef}
                className="mb-1 flex items-center gap-2 overflow-x-auto pb-1 scrollbar-thin scrollbar-thumb-gray-700 no-print"
              >
                <span className="text-[10px] text-gray-500 uppercase font-bold shrink-0 mr-2">
                  Wajah:
                </span>
            ...
            {locationBank && locationBank.length > 0 && (
              <div
                ref={locSelectorRef}
                className="mb-1 flex items-center gap-2 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-gray-700 no-print"
              >
                <span className="text-[10px] text-gray-500 uppercase font-bold shrink-0 mr-1.5">
                  Lokasi:
                </span>
            ...
            >
              {isGeneratingImg ? (
                <>
                  <LoaderIcon className="w-4 h-4" /> GENERATING...
                </>
              ) : (
                <>
                  <ImageIcon /> GENERATE VISUAL
                </>
              )}
            </button>
            {imgError && (
          ...
          <div className="flex flex-col gap-2 mt-2 pt-4 border-t border-gray-800 border-dashed">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => {
                  fallbackCopyTextToClipboard(motionPrompt);
                }}
                className="text-gray-500 hover:text-emerald-400 no-print"
          ...
          <div className="flex flex-col gap-2 mt-2 pt-4 border-t border-gray-800 border-dashed">
            ...
            >
              {isGeneratingAudio ? (
                <>
                  <LoaderIcon className="w-4 h-4" /> GENERATING VOICE...
                </>
              ) : (
                <>
                  <VolumeIcon /> GENERATE AUDIO
                </>
              )}
            </button>
            {audioError && (
      ...
      <ImageLightbox
        ...
        imageUrl={imageUrl}
        onDownload={() =>
          handleDownloadUpscaled(imageUrl, `scene-${sceneNum}-HD.png`)
        }
    ...
    if (part.type === 'markdown') {
      ...
      return (
        <StoryboardHeaderWidget
          ...
          setCharacterBank={setCharacterBank}
          locationBank={locationBank}
          setLocationBank={setLocationBank}
          formData={formData}
          sections={sections}
        />
      );
...
export default function App() {
  ...
  const handleSaveProject = async () => {
    ...
    try {
      ...
      if (err.message && err.message.includes('1048487 bytes')) {
        showToast(
          'Gagal menyimpan: Total ukuran gambar melebihi batas (1MB). Hapus beberapa gambar.',
          'error'
        );
      } else {
  ...
  const parseXMLResponse = (xmlString) => {
    try {
      const sectionRegex =
        /<section\s+id="([^"]+)"\s+title="([^"]+)">\s*<content>([\s\S]*?)<\/content>\s*<\/section>/gi;
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setError(
        err.message || 'Gagal menghasilkan pipeline. Silakan coba lagi.'
      );
    } finally {
      setIsGenerating(false);
    }
  ...
  const handleExportPDF = () => {
    if (typeof window.html2pdf !== 'undefined') {
      showToast(
        'Memproses PDF dalam Tema Terang... Mohon tunggu sebentar.',
        'success'
      );
      ...
      const fileName = `Film_Studio_Pipeline_${
        formData.idea.substring(0, 15).replace(/[^a-zA-Z0-9]/g, '_') ||
        'Project'
      }.pdf`;
      ...
      showToast(
        "Pilih 'Save as PDF' (Simpan sebagai PDF) di menu Printer.",
        'success'
      );
      setTimeout(() => window.print(), 1000);
    }
  ...
  if (!isAppReady) {
    return (
      <div className="min-h-screen bg-[#030712] flex flex-col items-center justify-center text-white selection:bg-blue-500/30 overflow-hidden relative">
        ...
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-purple-600/10 rounded-full blur-[80px] animate-pulse"
        ...
        <div className="relative flex flex-col items-center z-10 w-full px-6">
          ...
          <h1
            className="text-3xl md:text-5xl font-black tracking-[0.3em] text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-500 mb-6 text-center"
          ...
          <div className="w-64 max-w-full h-1 bg-gray-900 rounded-full overflow-hidden relative shadow-[0_0_15px_rgba(59,130,246,0.3)]">
            <div
              className="h-full bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 w-1/2 absolute top-0 left-0 rounded-full"
              style={{ animation: 'slideRight 1.5s infinite ease-in-out' }}
            ></div>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-black text-gray-200 font-sans selection:bg-blue-500/30">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 uppercase tracking-widest">
                Cinematic Production Pipeline Generator
              </p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col lg:flex-row gap-8">
        ...
        <div className="lg:w-1/3 xl:w-1/4 shrink-0 no-print">
          <div className="bg-gray-950 border border-gray-800 p-6 rounded-xl shadow-2xl sticky top-28">
            ...
            <div className="space-y-1">
              ...
              <InputField
                label={
                  <>
                    Lokasi Setting{' '}
                    <span className="text-gray-600 normal-case ml-1 font-normal">
                      (Optional)
                    </span>
                  </>
                }
            ...
            <button
              onClick={handleGenerate}
              disabled={isGenerating}
              className={`mt-8 w-full py-3 px-4 border border-transparent rounded-lg shadow-sm text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 focus:ring-offset-gray-900 transition-all flex justify-center items-center gap-2 ${
                isGenerating
        ...
        <div className="lg:w-2/3 xl:w-3/4 print-expand" ref={outputRef}>
          {sections.length === 0 && !isGenerating && (
            <div className="h-full min-h-[60vh] flex flex-col items-center justify-center border-2 border-dashed border-gray-800 rounded-xl bg-gray-950/50 text-gray-500 p-8 text-center no-print">
              ...
              <h3 className="mt-4 text-xl font-medium text-gray-400">
                Menunggu Input Proyek
              </h3>
              <p className="mt-2 text-sm max-w-md">
                Isi form di sebelah kiri dan klik Generate untuk menghasilkan
                dokumen pipeline produksi film komprehensif berteknologi AI.
              </p>
            </div>
          )}
          ...
          {isGenerating && (
            <div className="h-full min-h-[60vh] flex flex-col items-center justify-center border border-gray-800 rounded-xl bg-gray-950 p-8 text-center no-print">
              ...
              <p className="mt-2 text-blue-400 text-sm">
                AI sedang menulis skenario, mendesain storyboard, dan merancang
                pengaturan visual. Proses ini memakan waktu sekitar 15-30 detik.
              </p>
            </div>
          )}
          ...
          {sections.length > 0 && !isGenerating && (
            <div
              id="export-container"
              className="space-y-6 bg-black sm:bg-transparent rounded-xl"
            >
              ...
              <div className="flex justify-between items-end mb-8 border-b border-gray-800 pb-4">
                <div>
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    Generated by AI Film Studio
                  </p>
                </div>
                <div className="flex gap-2 no-print">
                  <button
                    onClick={handleExportPDF}
                    className="p-2 bg-emerald-900/30 hover:bg-emerald-800/50 border border-emerald-700/50 rounded text-emerald-300 transition-colors flex items-center gap-2 text-sm font-medium shadow-inner"
                  ...
                  <button
                    onClick={handleCopy}
                    className="p-2 bg-gray-900 hover:bg-gray-800 border border-gray-700 rounded text-gray-300 transition-colors flex items-center gap-2 text-sm font-medium"
              ...
              <div className="grid grid-cols-1 2xl:grid-cols-2 gap-6 print-grid-block">
                {sections.map((section, index) => {
                  ...
                  const isWideContent =
                    ['1', '2', '7', '8', '15'].includes(section.id) ||
                  ...
                  return (
                    <div
                      key={section.id}
                      className={`bg-gray-950 border border-gray-800 rounded-xl shadow-lg flex flex-col group transition-all duration-300 hover:border-gray-600 hover:shadow-2xl print-card break-inside-avoid ${
                        isWideContent
                          ? '2xl:col-span-2 print-col-span-2'
                          : 'col-span-1'
                      }`}
                    >
                      <div className="bg-gray-900/50 border-b border-gray-800 px-6 py-4 flex items-center gap-3 shrink-0 print-header">
                        ...
                        <h3 className="text-lg font-bold tracking-wide text-gray-100 uppercase">
                          {section.title}
                        </h3>
                      </div>
      ...
      {toast && (
        <div className="fixed bottom-6 right-6 z-50 no-print">
          <div
            className={`px-6 py-3 rounded-lg shadow-2xl border flex items-center gap-3 font-medium text-sm backdrop-blur-md transition-all duration-300
            ${
              toast.type === 'error'
              ...
              {!user ? (
                ...
                <div className="grid gap-4">
                  {savedProjects.map((project) => (
                    <div
                      key={project.id}
                      className="group relative bg-gray-900 border border-gray-800 hover:border-blue-500/50 rounded-lg p-4 transition-all hover:bg-gray-900/80 cursor-pointer flex justify-between items-center"
                      ...
                      <div>
                        ...
                        <div className="flex gap-3 text-xs text-gray-500">
                          <span className="flex items-center gap-1">
                            <FilmIcon /> {project.formData.genre}
                          </span>
                          <span>•</span>
                          <span>
